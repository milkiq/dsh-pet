...
// 条目级字段（animations / animationWeights / eventsRefreshSec）吹进每只实例。
// 不依赖 React/DOM；host 因 DSH 单文件加载约束不 import 本目录。
import type { Animations, Pet, PetDisplay, Weights } from './types';
 ...
 *  条目级字段（animations / animationWeights / eventsRefreshSec——合并器已填默认）吹进每只实例；
