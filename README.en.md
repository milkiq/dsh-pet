...
# ③ Build + inject the playback format (webm, the only published format)
...
> Note: only `prepare:webm` produces an installable lib (it injects the `.webm` extension); a bare `tsdown` build leaves the placeholder in place and animations won't play.
...
### ②.5 🍎 Safari/HEVC pipeline (kept in-repo, not part of publishing — for forks)

Since v0.3 the plugin publishes **a single webm format** (VP9-alpha) shared by the browser overlay (Chrome/Edge/Firefox) and the desktop mode (Electron = Chromium); no second asset set is shipped, and the **host thumb route has dropped the `.mov` branch** (`dsh-pet/src/host/index.ts`). Safari does not support webm alpha (black background) and only plays **HEVC-with-Alpha mov** — that encoder (`hevcWithAlpha`, AVFoundation) exists **only on macOS**. If you need Safari compatibility, **fork the repo, enable the kept pipeline and re-add the `.mov` branch yourself**:
...
- workflow: `.github/workflows/hevc-alpha.yml` (manual trigger via `workflow_dispatch`; macOS runner batch transcode)
- encoder script: `scripts/encode_hevc_alpha.sh` (ffmpeg only decodes the webm → raw BGRA frame pipeline → Swift `hevc_alpha_encoder.swift` via AVAssetWriter `hevcWithAlpha` native API) + `scripts/check_alpha.py` (artifact verification)
- input: `dsh-pet/assets/webm/*.webm`; output written directly back to `dsh-pet/assets/mov/` (pipeline output dir — not committed, not published)
- to enable: sync the mov assets into the package and restore dual-format support yourself (`prepare.js` is now webm-only and the host thumb route has dropped `.mov` — both mov branches live in git history)

### ③ Animations → Plugin

```sh
# Sync the step04 playback variants into the plugin package (webm is a plain cp)
cp step04/*.webm dsh-pet/assets/webm/   # playback format (VP9-alpha)
...
### 🎯 Publishing (single webm format)

The plugin publishes one npm package, one format (webm); `files` always carries only `assets/webm` (shared by browser and desktop mode, minimal payload):

```sh
cd dsh-pet
npm run prepare:webm   # build + inject .webm → converge the files list (including the desktop runtime)
npm publish --tag latest
```

- The client does no runtime browser sniffing — the extension is injected at publish time (`prepare:webm` → `.webm`)
- Need a Safari/HEVC build? See ②.5 above — fork the repo and enable the kept pipeline
...
│   ├── assets/webm/         #   640×360 VP9-alpha playback animations (the only published format)
...
│   └── scripts/prepare.js   # pre-publish micro-adjust (build + inject playback extension .webm)
...
All animations (640×360, the actual assets the plugin plays) — GIF previews live in the repo at `dsh-pet/assets/preview/` (rendered via raw links, pinyin filenames for cross-platform safety); the full transparent videos live in the plugin package under `dsh-pet/assets/webm/` (VP9-alpha, the only published format):
